    ...
    return (
        posts.filter(filter).map((post) => (
            ...
            post.post.childids !== -1 ? (
                    ...
                    post.post.repost ?
                        ParentRepost(post.post, post.user, user, posts, openComments, renderTextWithHashtags, setUser, setPosts, setOpenComments)
...
export const ParentRepost = (post: Post, postUser: User, loggedInUser: User | null, posts: Array<PackagedPost>, openComments: Set<string>, renderTextWithHashtags: (text: string) => any, setUser: (user: User) => any, setPosts: (p: Array<PackagedPost>) => any, setOpenComments:(c: Set<string>) => any) => {
    return (
        <div
            key={post.id}
            className="bg-background-secondary p-4 rounded-lg mb-6 relative truncate"
        >
            ...
            <div className="flex flex-row gap-1 mb-2 fill-text-secondary items-center ml-14">
                ...
                <Link to={"/profile/" + post.repostuser} className="hover:underline decoration-text-secondary">
                <p className="text-text-secondary text-sm">Reposted by <b>{post.repostDN || post.repostuser}</b></p>
                </Link>
            </div>
