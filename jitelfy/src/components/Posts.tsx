...
export const mapProfilePosts = (posts: Array<PackagedPost>, profileUser: User, user: User | null, showPosts: Boolean, showComments: Boolean, openComments: Set<string>, renderTextWithHashtags: (text: string) => any, setUser: (user: User) => any, setPosts: (p: Array<PackagedPost>) => any, setOpenComments:(c: Set<string>) => any) => {
    return (
        <div>
            {/* All user posts */}
            { showPosts && (
                posts.filter((post: PackagedPost) => post.post.parentid === "000000000000000000000000")).map((post) => (
                    ...
                    post.post.childids !== -1 ? (
                        /* Is the post a repost? */
                            post.user.id === profileUser.id ?
                                ...
                                : ParentRepost(post.post, post.user, profileUser, user, posts, openComments, renderTextWithHashtags, setUser, setPosts, setOpenComments)
                        )
                        : <p className="text-text-secondary text-sm mb-4 mt-2">This post has been deleted.</p>
                ))
            }
...
export const ParentRepost = (post: Post, postUser: User, profileUser: User, loggedInUser: User | null, posts: Array<PackagedPost>, openComments: Set<string>, renderTextWithHashtags: (text: string) => any, setUser: (user: User) => any, setPosts: (p: Array<PackagedPost>) => any, setOpenComments:(c: Set<string>) => any) => {
    ...
    return (
        ...
        >
            {/* Reposted by "user" */}
            <div className="flex flex-row gap-1 mb-2 fill-text-secondary items-center ml-14">
                ...
                <Link to={"/profile/" + profileUser.username} className="hover:underline decoration-text-secondary">
                <p className="text-text-secondary text-sm">Reposted by <b>{profileUser.displayname}</b></p>
                </Link>
            </div>
